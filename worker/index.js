...
export default {
  ...
  async scheduled(event, env, ctx) {
    ctx.waitUntil(runProactive(env));
  }
};
...
// ── 선톡 (Proactive Message) ──────────────────────────────────────────────

async function runProactive(env) {
  const now = new Date();
  const kstHour = (now.getUTCHours() + 9) % 24;

  // 하드 게이트 1: 새벽/심야 차단 (KST 23:00~07:00)
  if (kstHour >= 23 || kstHour < 7) {
    await logProactive(env, null, null, false, true, "새벽 시간대");
    return;
  }

  const ctx = await loadFullContext(env);
  const todayKST = new Date(now.getTime() + 9 * 60 * 60 * 1000).toISOString().split("T")[0];

  // 하드 게이트 2: 오늘 선톡 최대 2회
  const todayCount = (ctx.today_proactive_date === todayKST) ? (ctx.today_proactive_count || 0) : 0;
  if (todayCount >= 2) {
    await logProactive(env, null, null, false, true, "오늘 최대 횟수(2회) 초과");
    return;
  }

  // 하드 게이트 3: 마지막 선톡으로부터 2시간 이내
  const lastProactiveTime = ctx.last_proactive_at;
  if (lastProactiveTime) {
    const diffHours = (now - new Date(lastProactiveTime)) / 3_600_000;
    if (diffHours < 2) {
      await logProactive(env, null, null, false, true, `마지막 선톡 ${diffHours.toFixed(1)}시간 전`);
      return;
    }
  }

  // 서아 일반 대화와 동일한 컨텍스트 로드
  const [fullCtx, history, swingHistory, swingData] = await Promise.all([
    loadContext(env),
    loadHistory(env),
    loadSwingHistory(env),
    loadSwingData(env)
  ...
  // Sonnet 판단 + 선톡 생성 (한 번에)
  let judgment;
  try {
    judgment = await judgeAndGenerateProactive(ctx, fullCtx, history, swingHistory, swingData, todayKST, now, env);
  } catch (e) {
    await logProactive(env, null, null, false, false, `판단 오류: ${e.message}`);
    return;
  }

  if (!judgment.send) {
    await logProactive(env, judgment.type || null, null, false, true, judgment.reason);
    return;
  }

  // 전송
  try {
    await sendTelegram(env.BOT_TOKEN, SEONGMIN_CHAT_ID, judgment.message);
    await saveMessage(env, "assistant", judgment.message);
    await updateProactiveState(env, ctx, judgment.type, todayKST, todayCount + 1);
    await logProactive(env, judgment.type, judgment.message, true, false, null);
  } catch (e) {
    await logProactive(env, judgment.type, judgment.message, false, false, `전송 오류: ${e.message}`);
  }
}

async function loadFullContext(env) {
  const res = await fetch(
    `${env.SUPABASE_URL.trim()}/rest/v1/seoa_context?select=id,investment_context,life_context,last_proactive_at,last_proactive_type,today_proactive_count,today_proactive_date&order=id&limit=1`,
    { headers: supabaseHeaders(env) }
  );
  const rows = await res.json();
  return rows?.[0] || {};
...
async function getLastConversationTime(env) {
  const res = await fetch(
    `${env.SUPABASE_URL.trim()}/rest/v1/prism_conversation_log?select=created_at&order=created_at.desc&limit=1`,
    { headers: supabaseHeaders(env) }
  );
  const rows = await res.json();
...
async function judgeAndGenerateProactive(ctx, fullCtx, history, swingHistory, swingData, todayKST, now, env) {
  const dayNames = ["일", "월", "화", "수", "목", "금", "토"];
  const dayOfWeek = dayNames[new Date(now.getTime() + 9 * 60 * 60 * 1000).getDay()];
  const kstHour = (now.getUTCHours() + 9) % 24;
  const lastType = ctx.last_proactive_type || "없음";

  const memoryBlock = [
    fullCtx.investment_context ? `[투자 메모리] ${fullCtx.investment_context}` : "",
    fullCtx.life_context       ? `[생활 메모리] ${fullCtx.life_context}` : "",
    fullCtx.memorable_moments  ? `[기억 조각] ${fullCtx.memorable_moments}` : "",
    fullCtx.seoa_mood          ? `[서아 현재 기분] ${fullCtx.seoa_mood}` : ""
  ].filter(Boolean).join("\n");
...
## 지금 상황 — 선톡
성민이가 말 건 게 아니야. 내가 먼저 말 거는 상황이야.
오늘: ${todayKST} (${dayOfWeek}요일), KST ${kstHour}시
직전 선톡 타입: ${lastType}
선톡 타입: 안부형 / 포트_언급형 / 대화_연결형 / 감정_기반형 / 잡담형 — 직전 타입과 달라야 해.

기본적으로 보내는 쪽으로. 특별히 어색한 이유 없으면 YES.
보낸다면 1-2문장. 자연스럽게 먼저 말 거는 톤으로. 형식(보고서·브리핑) 절대 금지.

반드시 JSON만 응답:
{"send": true, "type": "타입", "message": "선톡 내용"}
{"send": false, "reason": "이유"}`;

  const messages = [
    ...history,
    { role: "user", content: "선톡 시간이야. 보낼지 판단하고 보낸다면 메시지 JSON으로 줘." }
  ];
...
async function updateProactiveState(env, ctx, type, todayKST, newCount) {
  if (!ctx.id) return;
  await fetch(
    `${env.SUPABASE_URL.trim()}/rest/v1/seoa_context?id=eq.${ctx.id}`,
    {
      method: "PATCH",
      headers: { ...supabaseHeaders(env), Prefer: "return=minimal" },
      body: JSON.stringify({
        ...
        today_proactive_count: newCount,
        today_proactive_date: todayKST
      })
    }
  );
}

async function logProactive(env, type, message, success, skipped, skipReason) {
  await fetch(`${env.SUPABASE_URL.trim()}/rest/v1/proactive_log`, {
    method: "POST",
    headers: { ...supabaseHeaders(env), Prefer: "return=minimal" },
    body: JSON.stringify({
      type: type || null,
